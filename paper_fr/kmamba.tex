...
\subsection{Architecture : Orchestration/Topologie/Kernels}

K-Mamba est organisé autour d'une stricte séparation en trois couches :

\begin{itemize}
  \item \textbf{Orchestration (src/)} : logique modèle, API, boucle d'entraînement —
        recherche d'embedding, empilement des blocs Mamba, tête de langage,
        entrées/sorties de points de contrôle.
        Tout en C trivial ($\leq$10 lignes par opération).
  \item \textbf{Topologie (src/km\_topology.c)} : indexation ND, scheduling wavefront —
        primitives communes pour les strides ND et l'ordonnancement causal. Réutilisable par scan et convolution.
  \item \textbf{Kernels (kernels/, cuda/, cpu/)} : moteur de calcul —
        GEMM/GEMV en C pur inline, activations, Conv1D/ND, optimiseurs (AdamW, MUON),
        noyaux CPU (ASM AVX2) et CUDA. Toutes les boucles critiques.
\end{itemize}
