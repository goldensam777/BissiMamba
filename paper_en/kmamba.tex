...
\subsection{Architecture: Orchestration/Topology/Kernels}

K-Mamba is organized around a strict three-layer separation of concerns:

\begin{itemize}
  \item \textbf{Orchestration (src/)}: model logic, API, training loop —
        embedding lookup, MambaBlock stacking, LM head,
        checkpoint I/O. All trivial C code ($\leq$10 lines per operation).
  \item \textbf{Topology (src/km\_topology.c)}: ND indexing, wavefront scheduling —
        common primitives for ND strides and causal ordering. Reusable by scan and convolution.
  \item \textbf{Kernels (kernels/, cuda/, cpu/)}: compute engine —
        GEMM/GEMV pure C inline, activations, Conv1D/ND, optimizers (AdamW, MUON),
        CPU (ASM AVX2) and CUDA kernels. All hot-path loops.
\end{itemize}

This separation ensures that changing the compute backend
(e.g., porting to ARM NEON or AVX-512) requires only modifying
\texttt{kernels/}, leaving model logic untouched.
...
\subsection{Pure C GEMM Kernel}

The GEMM kernel (\texttt{kernels/gemm\_f32.c}) uses a compiler-optimized
inline C implementation: optimized loops with local accumulators
and cache-friendly access patterns. The pure C approach lets the compiler
exploit AVX2/FMA automatically via \texttt{-O3 -march=native}.
...
While the wavefront-unified ConvND (Section~\ref{sec:implementation}) provides
a single-kernel dense $K^N$ implementation, we also implement the
classical \emph{separable} convolution as a cascade of $N$ 1D convolutions.
This reduces computational complexity from $O(K^N)$ to $O(N \cdot K)$ per position.

\subsection{Implementation}

Each axis is processed sequentially using the same wavefront infrastructure:

\begin{verbatim}
// Cascade: N 1D convolutions with ping-pong buffers
for axis in 0..N-1:
    convnd_separable_1d_wavefront(input, output, kernel[axis], axis)
    swap(input, output)
\end{verbatim}

The separable approach requires $N \cdot K$ multiplications per spatial position
versus $K^N$ for the dense kernel.

\subsection{CPU vs GPU Performance}

Surprisingly, the performance winner depends on the execution platform:

\begin{table}[htbp]
\centering
\caption{ConvND performance: Dense $K^N$ vs Separable cascade (2D grids, $D{=}64$, $K{=}3$).}
\label{tab:convnd}
\begin{tabular}{lrrrr}
\toprule
Platform & Grid & Dense (ms) & Separable (ms) & Speedup \\
\midrule
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{../figures/convnd_dense_vs_separable_en.pdf}
\caption{ConvND Dense vs Separable performance on CPU (2D grids, $D{=}64$, $K{=}3$).}
\label{fig:convnd_cpu}
\end{figure}

On CPU, separable wins due to reduced arithmetic ($6$ vs $9$ multiplications).
On GPU, dense wins because (1) single kernel launch avoids launch overhead,
(2) better memory coalescing, and (3) the GPU masks computational complexity
via massive parallelism. This motivates providing both implementations.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{../figures/convnd_cuda_dense_vs_separable.pdf}
\caption{ConvND Dense vs Separable performance on GPU (NVIDIA MX450).}
...
with five key contributions: (1) a formal proof that $d \times d$
...
same work; (4) separable ConvND with empirical characterization showing
algorithmic optimality depends on execution platform (CPU vs GPU);
(5) a zero-dependency C/ASM/CUDA implementation of the full
