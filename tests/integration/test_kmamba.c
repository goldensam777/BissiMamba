/*
 * test_kmamba.c — Tests de bout en bout KMamba
 *
 * Phase 3 : Tests de bout en bout KMamba (training/inference)
 * Objectif : Valider le modèle complet avec embedding, MambaBlocks, LM head
 */
...
typedef struct {
    ...
    /* MambaBlocks */
    SimpleMatrix *mamba_blocks;  // Tableau de blocks
    SimpleMatrix *block_configs;   // Configurations des blocks
...
SimpleKMamba* simple_kmamba_create(const SimpleKMambaConfig *config);
void simple_kmamba_free(SimpleKMamba *kmamba);
void simple_kmamba_init(SimpleKMamba *kmamba, unsigned long seed);
void simple_kmamba_forward(SimpleKMamba *kmamba, const uint8_t *tokens, float *logits);
float simple_kmamba_train_step(SimpleKMamba *kmamba, const uint8_t *tokens, const uint8_t *targets);
void simple_kmamba_enable_training(SimpleKMamba *kmamba, float lr, float weight_decay);
