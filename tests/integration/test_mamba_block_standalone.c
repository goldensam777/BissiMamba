/*
 * test_mamba_block_standalone.c — Tests MambaBlock standalone
 *
 * Phase 2 : Tests d'intégration MambaBlock (forward/backward)
 * Version standalone avec implémentations simulées pour valider l'approche
 */
...
/* ============================================================
 * Structures de données pour MambaBlock simulé
 * ============================================================ */
...
/* ============================================================
 * Implémentation simulée de MambaBlock
 * ============================================================ */
...
static void simple_mamba_block_forward(SimpleMambaBlock *block, float *output, const float *input, size_t batch_size) {
    ...
    for (size_t t = 0; t < L; t++) {
        ...
        for (size_t m = 0; m < M; m++) {
            ...
            float h_old = block->scan_h.data[m];
            float h_new = a * h_old + b;
            block->scan_h.data[m] = h_new;
    ...
    for (size_t t = 0; t < L; t++) {
        for (size_t d = 0; d < D; d++) {
            float sum = 0.0f;
            for (size_t m = 0; m < M; m++) {
                sum += block->scan_h.data[m] * block->W_out.data[d * M + m];
            }
            output[t * D + d] = sum;
        }
    }
...
int main() {
    printf("=== MambaBlock Standalone Test Suite ===\n");
    printf("Testing MambaBlock integration with simulated implementation\n\n");
