/*
 * test_activations_standalone.c — Tests activations sans dépendances ASM
 * Version scalaire pour valider les fonctions de référence
 */
...
/* ============================================================
 * Références C pures (implémentations optimatrix)
 * ============================================================ */
...
/* Versions vectorielles scalaires (pour tester l'approche) */
static void silu_f32_scalar(float *x, float *y, long n) {
    for (long i = 0; i < n; i++) {
        y[i] = silu_reference(x[i]);
    }
}

static void sigmoid_f32_scalar(float *x, float *y, long n) {
    for (long i = 0; i < n; i++) {
        y[i] = sigmoid_reference(x[i]);
    }
}

static void softplus_f32_scalar(float *x, float *y, long n) {
    for (long i = 0; i < n; i++) {
        y[i] = softplus_reference(x[i]);
    }
}
...
static int test_silu_range() {
    ...
    if (!result) {
        printf("FAIL: SiLU range test failed\n");
        return 0;
    }
...
static int test_sigmoid_basic() {
    ...
    /* Test */
    sigmoid_f32_scalar(input, test, (long)n);
    ...
    printf("Scalar:    [%.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %.6f]\n", 
           test[0], test[1], test[2], test[3], test[4], test[5], test[6]);
    
    int result = compare_vectors(ref, test, n, EPSILON);
    if (!result) {
        printf("FAIL: Sigmoid basic values failed\n");
        return 0;
    }
...
static int test_sigmoid_extremes() {
    ...
    printf("Scalar:    [%.10f, %.10f, %.10f, %.10f]\n", 
           test[0], test[1], test[2], test[3]);
    
    int result = compare_vectors(ref, test, n, EPSILON);
    if (!result) {
        printf("FAIL: Sigmoid extremes failed\n");
        return 0;
    }
...
static int test_softplus_basic() {
    ...
    /* Test */
    softplus_f32_scalar(input, test, (long)n);
    ...
    int result = compare_vectors(ref, test, n, EPSILON);
    if (!result) {
        printf("FAIL: Softplus basic values failed\n");
        return 0;
    }
...
static void benchmark_activations() {
    ...
    for (int iter = 0; iter < iterations; iter++) {
        silu_f32_scalar(input, output, (long)n);
    }
    ...
    clock_gettime(CLOCK_MONOTONIC, &start);
    for (int iter = 0; iter < iterations; iter++) {
        sigmoid_f32_scalar(input, output, (long)n);
    }
    clock_gettime(CLOCK_MONOTONIC, &end);
    ...
    clock_gettime(CLOCK_MONOTONIC, &start);
    for (int iter = 0; iter < iterations; iter++) {
        softplus_f32_scalar(input, output, (long)n);
    }
    clock_gettime(CLOCK_MONOTONIC, &end);
...
int main() {
    printf("=== Activations Standalone Test Suite ===\n");
    printf("Testing SiLU, Sigmoid, Softplus scalar implementations\n\n");
