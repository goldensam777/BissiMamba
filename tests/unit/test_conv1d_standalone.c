/*
 * test_conv1d_standalone.c — Tests Conv1D sans dépendances externes
...
static int test_conv1d_causality() {
    ...
    float kernel[9] = {0.0f};  // Kernel nul
    kernel[0 * D + 1] = 1.0f;  // Seul k=0, d=1 = 1 (kernel identité)
    float bias[3] = {0.0f};
    ...
    for (long l = 0; l < L; l++) {
        for (long d = 0; d < D; d++) {
            float expected = 0.0f;
            /* Pour l >= 2, l'élément d=1 doit voir l'input à l=2 */
            if (l >= 2 && d == 1) {
                expected = 1.0f;
            }
            if (fabsf(output_ref[l * D + d] - expected) > EPSILON) {
                printf("Debug: l=%ld, d=%ld, output=%.6f, expected=%.6f\n",
...
static int test_conv1d_padding() {
    ...
    float kernel[6] = {0.0f};  // Kernel nul
    kernel[0 * D + 0] = 1.0f;  // Seul k=0, d=0 = 1 (identité sur d=0)
    float bias[2] = {0.0f};
    ...
    for (long l = 0; l < L; l++) {
        for (long d = 0; d < D; d++) {
            float expected = 0.0f;
            /* Seul d=0 reçoit l'input correspondant */
            if (d == 0) {
                expected = input[l * D + d];
            }
            if (fabsf(output_ref[l * D + d] - expected) > EPSILON) {
...
int main() {
    printf("=== Conv1D Standalone Test Suite ===\n");
