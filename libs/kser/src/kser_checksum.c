/* ============================================================================
 * kser_checksum.c — SHA256 implementation (zero-dependency)
...
/* Streaming SHA256 over a FILE* — hashes bytes [0, end_pos) */
int kser_sha256_file(FILE* fp, long end_pos, uint8_t hash[32]) {
    ...
    while (pos < end_pos) {
        ...
        if ((long)want > end_pos - pos) want = (size_t)(end_pos - pos);
